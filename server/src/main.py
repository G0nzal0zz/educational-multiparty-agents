...
MAX_BYTES = SAMPLE_RATE * BYTES_PER_SAMPLE * SECONDS
...
async def _whisper_stt_stream(
    ...
    """
    Collect exactly n seconds of audio, then run Whisper once.
    """

    collected = bytearray()

    # TODO: Implement a proper turn-taking approach (VAD?)
    ...
    print("Converting to wav_buffer")
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav") as tmp:
        ...
        transcription = await asyncio.to_thread(
            whisperSTT.transcribe, tmp.name, language="en", fp16=False
        )
    print("Transcription: ", transcription)

    yield STTOutputEvent.create(transcription["text"])
...
async def _ollama_agent_stream(
    ...
    async for event in event_stream:
        ...
        if event.type == "stt_output":
            ...
            prompt = build_ollama_prompt(event.transcript)
            ...
            async for message in stream:
                print("Ollama stream: ", message.text)
